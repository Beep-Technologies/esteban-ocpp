...
type OCPP16ChargePoint struct {
	...
	currentConnectorTransactions map[int]int // key: connectorId, value: transactionId
...
func NewOCPP16ChargePoint(
	...
	return &OCPP16ChargePoint{
		...
		currentConnectorTransactions: make(map[int]int),
		logger:                       logger,
	}
}

func (c *OCPP16ChargePoint) GetTransactionConnectorID(tid int) int {
	for cnid, trid := range c.currentConnectorTransactions {
		if trid == tid {
			return cnid
		}
	}
